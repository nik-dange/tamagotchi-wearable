...
SCREEN_HEIGHT = 520
##defined filepaths 
##for sound effects and commands
filepath="./Design Challenge/commands.csv"
soundpath1="./sounds/buy2.wav"
soundpath2="./sounds/eb_winboss.wav"
soundpath3="./sounds/die.wav"
soundpath4="./sounds/skyrunner.wav" 
soundpath5="./sounds/attack1.wav" 
#global wallet variable for tamagotchi
wallet=0
...
##reads the most recent value of the file
def filegetter(filepath): 
  data=np.genfromtxt(filepath,delimiter=",") 
  value=data[-1]  
  return value

#used for reading the last value obtained for the wallet, and comparing it to the current value
amount=filegetter(filepath)
...
def main():
    ...
    ##UPDATE
    #defined sounds 
    moneysound=pygame.mixer.Sound(soundpath1)
    agingsound=pygame.mixer.Sound(soundpath2)
    deathsound=pygame.mixer.Sound(soundpath3)
    beepsound=pygame.mixer.Sound(soundpath4)
    movesound=pygame.mixer.Sound(soundpath5)
    ##UPDATE
    ...
    # Tamagotchi
    pet = {'hunger':0, 'energy':256, 'waste':0, 'age':0, 'happiness':0,'wallet':0} 
    ...
    while True:
        ...
        if button == 0:
            ...
            ##sound for moving between buttons
            sound(movesound)
            if stage > 0 or selid == 2:
                if selid == 0 and wallet>0:
                    eating = True
                    overlay_anim = OVERLAY_EAT
                    ol_frame = 0
                    has_overlay = True
                    wallet-=2
                    ##sounds for using wallet
                    sound(moneysound)
                elif selid == 1 and wallet>0:
                    cleaning = True
        ...
        if update_game:
            if stage == 0 and pet['age'] > AGE_HATCH:
                ...
                has_overlay = False
                ##UPDATE
                #sound for advancing in maturity
                sound(agingsound)
                ##UPDATE
            ...
            if not sleeping and not cleaning and not eating and not dead:
                if pet['waste'] >= WASTE_EXPUNGE:
                    ...
                    #sound for tamagotchi in state where it is not being well kept/ life is in danger
                    sound(beepsound)
                    has_overlay = True
                if not dead:
        ...
        debug = (('AGE: %s', 'HUNGER: %s', 'ENERGY: %s', 'WASTE: %d', 'HAPPINESS: %s','WALLET: %d'), \
                 ('age', 'hunger', 'energy', 'waste', 'happiness','wallet'))
        for pos, y in enumerate(i for i in range(70, 130, 10)):
